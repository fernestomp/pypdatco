...
class PDC:
    ...
    def __fill_pmu_info(self):
        ...
        for pmu_num in range(1,num_pmus+1):
            ...
            dict_PMU['FORMAT_FREQ_DFREQ'] = self.dfCFG2[self.dfCFG2['field']=='PMU{}_FORMAT_FREQ_DFREQ'.format(pmu_num)]['value'].values[0]
            dict_PMU['FORMAT_ANALOG'] = self.dfCFG2[self.dfCFG2['field']=='PMU{}_FORMAT_ANALOG'.format(pmu_num)]['value'].values[0]

            dict_PMU['PHASOR_FORMAT'] = self.dfCFG2[self.dfCFG2['field']=='PMU{}_FORMAT_PHASOR'.format(pmu_num)]['value'].values[0]
            dict_PMU['PHASOR_NOTATION'] = self.dfCFG2[self.dfCFG2['field']=='PMU{}_PHASOR_NOTATION'.format(pmu_num)]['value'].values[0]
            for pn, name in enumerate(lst_ph_names,1):
                dict_PMU['PHASOR{}_NAME'.format(pn)]=name
                dict_PMU['PHASOR{}_UNIT'.format(pn)] =self.dfCFG2[self.dfCFG2['field']=='PMU{}_PHASOR{}_UNIT'.format
                (pmu_num,pn)]['value'].values[0]
                dict_PMU['PHASOR{}_CONV_FACTOR'.format(pn)] =self.dfCFG2[self.dfCFG2['field']=='PMU{}_PHASOR{}_CONV_FACTOR'.format
                (pmu_num,pn)]['value'].values[0]
    ...
    def decode_cfg2(self, cgf2frame, verbose=False):
        ...
        dictDATAread = {'ACK': leadingbyteSYNC, 'frameType': frametypebits, 'frameTypeStr': strframetype,
            ...
            'FRACSEC': decFRACSEC, 'TIME_QUALITY': TIME_QUALITY, 'TIME_BASE': decTIME_BASE, 'NUM_PMU': decNUM_PMU,
            'STN': STNread, 'IDCODEsrc': decIDCODESRC, 'FORMAT': FORMATread, 'FORMAT_freq_dfreq': FORMAT_FREQ_DFREQ,
            'FORMAT_analog': FORMAT_ANALOG, 'FORMAT_phasor': FORMAT_PHASOR, 'PHASOR_notation': PHASOR_NOTATION,
            'PHNMR': decPHNMR, 'ANNMR': decANNMR, 'DGNMR': decDGNMR, 'PHASOR_names': PHASORNAMES,
            'ANALOG_names': ANALOGNAMES, 'DIGITAL_labels': DIGITALLABELS, 'PHUNIT': listPHUNIT,
            'PHUNIT_str': PHUNIT_str, 'PHUNIT_factor': PHUNIT_factor, 'ANUNIT': listANUNIT,
            'ANUNIT_factor': ANUNIT_factor, 'ANUNIT_value': ANUNIT_value, 'DIGUNIT': listDIGUNIT, 'FNOM': decFNOM,
            'CFGNT': decCFGCNT, 'DATA_RATE': decDATA_RATE, 'CHK': decCHK}
        # class values
    ...
    def decode_dataframe(self, data, verbose=False):
        ...
        dictDATAread = {'ACK': field1_1, 'frameType': DATAFRAMETYPEread, 'frameTypeStr': strDATAFRAMETYPEread,
            ...
            'PHASORS_magnitude': phasorMagnitude, 'PHASORS_angle': phasorAngle, 'FREQ': FREQread, 'DFREC': DFREQread,
            'ANALOG': ANALOGvalues, 'DIGITAL': DIGITALread, }
        ################################################################
        # PRINT VALUES
        if verbose == True:
...
#****************************************************************
        ...
        for index,row_pmu in self.dfPMU_info.iterrows():
            ...
            phasor_names = self.dfCFG2[self.dfCFG2['field']=='PMU{}_CHNAM_PHASORS'.format(index+1)]['value'].values[0]
                ...
                if row_pmu['PHASOR_NOTATION'] == 'polar':
                    for pn in range(1,number_of_phasors+1):
                        ...
                        dictDF_Decoded['PHASOR{}_UNIT'.format(pn)] = \
                            self.dfCFG2[self.dfCFG2['field'] == 'PMU{}_PHASOR{}_UNIT'.format(index+1,pn)]['value'].values[0]
        ...
        self.dataFrame = pd.DataFrame.from_records(lstPMU_Meas)
